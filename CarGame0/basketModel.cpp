...
static Mesh g_basket_mesh;

void InitBasketModel()
{
	// 3D데이터 파일 obj
	g_basket_mesh.ReadObjFormatFile("../Data/basket.obj");

	g_basket_mesh.TriangulateAndUniformizeBuffers();
	...
	g_basket_mesh.SetColor(0.40, 0.30, 0.10, 1.0);//전체적인 컬러를 모든꼭지점에대해 똑같이 해준다.
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 3 * g_basket_mesh.num_points(), g_basket_mesh.points(), GL_STATIC_DRAW);

	// Color VBO
	glGenBuffers(1, &g_car_vbo_color);
	glBindBuffer(GL_ARRAY_BUFFER, g_car_vbo_color);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 4 * g_basket_mesh.num_points(), g_basket_mesh.colors(), GL_STATIC_DRAW);

	// Normal VBO
	glGenBuffers(1, &g_car_vbo_normal);
	glBindBuffer(GL_ARRAY_BUFFER, g_car_vbo_normal);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 3 * g_basket_mesh.num_points(), g_basket_mesh.normals(), GL_STATIC_DRAW);


	// Index Buffer Object
	glGenBuffers(1, &g_car_index_buf);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, g_car_index_buf);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * 3 * g_basket_mesh.num_triangles(), g_basket_mesh.triangle_ids(), GL_STATIC_DRAW);
...
void DrawBasketModel()
{
	...
	glDrawElements(GL_TRIANGLES, g_basket_mesh.num_triangles() * 3, GL_UNSIGNED_INT, NULL);//진짜로 그림.
}
